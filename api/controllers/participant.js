...
export const getVotes = (req, res) => {
  ...
  const q =
    "SELECT max(n.`nickname`) as nickname, max(u.`username`) as username, max(v.`usid`) as `user`, max(v.`sugid`) as `sugid`,v.`rank` as `rank`, max(s.`suggestion`) as `suggestion`, max(p.`password`) as `password`, max(p.`phase`) as `phase`, max(s.`userid`) as `sugUser` FROM votes as v INNER JOIN suggestions as s ON s.`id`=v.`sugid` INNER JOIN polls as p ON p.`password`=s.`ppwd` INNER JOIN users as u ON u.`id`=s.`userid` INNER JOIN `nicknames` as n ON n.`user_id`=u.`id` WHERE v.`usid`=? AND p.`password`=? GROUP BY v.`rank`";
